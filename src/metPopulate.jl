 ...
 reader = open(FASTA.Reader, path)
    for record in reader
        ...
        try
            ...
            metAPIcalls.insert_API_taxa(lst[4], lst[5], lst[6], lst[7])
            inserted = true
        catch e
        ...
        if inserted == true
            ...
            resp = metAPIcalls.get_API_taxa(lst[4], lst[5], lst[6], lst[7])
            id = replace(resp, "[" => "")
            id = replace(id, "]" => "")

            metAPIcalls.insert_API_seqID(parse(Int64, id), FASTA.sequence(record), source, FASTA.identifier(record))
        else
...
function SraRunInfo_insert(local_path, download_path)
	...
	try
		metAPIcalls.insert_API_dataset(data_df.Run[1], data_df.Run[1], download_path)
	catch e
	...
	date = rsplit(data_df.Collection_date[1], "-")
	month = date[1]
	year = date[2]
	...
	id = metAPIcalls.get_API_dataset(data_df.Run[1])
...
function SraFASTQ_insert(sra_num, gene_region)
    ...
    dataset_id = metAPIcalls.get_API_dataset(sra_num)
    dataset_id = replace(dataset_id, "[" => "")
    dataset_id = replace(dataset_id, "]" => "")
    ...
    for seq in keys(seq_dict)
        try
            ...
            metAPIcalls.insert_API_asv(seq, string(floor(seq_dict[seq][1])), gene_region)
        catch e
        ...
        if inserted == 0
            asv_id = metAPIcalls.get_API_asv(seq)
            asv_id = replace(asv_id, "[" => "")
            asv_id = replace(asv_id, "]" => "")

            metAPIcalls.insert_API_asvAssignment(parse(Int64, asv_id), parse(Int64, dataset_id), convert(Int64, seq_dict[seq][2]))
        end
    end
